...
1.  To download the templates, you can either clone the repo with the git command below, or [**download**](https://github.com/FortinetCloudCSE/fortigate-aws-tgwconnect-active-active-cloudformation) the repo as a ZIP archive.  The templates themselves are located in the **/cloudformation folder**

```
git clone https://github.com/FortinetCloudCSE/fortigate-aws-tgwconnect-active-active-cloudformation.git
```
...
5.  On the Specify Details page, you will be prompted for a stack name and parameters for the deployment.  We are using the **'NewSecurityVPC_TGW-Connect_ActiveActive.template.json'** template which deploys a new VPC, gives options for deploying a new or working with an existing TGW, and deploys the FGTs as well.
...
11.  Here is an example of deploying a spoke VPC using the **'NewVPC_Spoke_TGW_DualAZ.template.json'** template.  Note we are referencing the relevant IDs for Transit Gateway and relevant TGW Route Tables created for us using the **'NewSecurityVPC_TGW-Connect_ActiveActive.template.json'** template.
